// ============================================================================
// COMPONENTE — COMPRAS AGENCIA (bandeja del Administrador de Bodegas)
// ============================================================================
...
import { Compra, ESTADO_COMPRA, EstadoCompraId } from '../models/compras-agencia.models';
import { ComprasAgenciaService } from '../services/compras-agencia.service';
import { ModalCrearSolicitudAgenciaComponent } from './modal-crear-solicitud-agencia/modal-crear-solicitud-agencia.component';
...
/** Clases de badge por estado — mismo criterio visual que getEstadoClase() en otros módulos */
...
@Component({
    selector: 'app-compras-agencia',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
        LucideAngularModule,
        ModalCrearSolicitudAgenciaComponent,
    ],
    templateUrl: './compras-agencia.component.html',
    styleUrls: ['./compras-agencia.component.css'],
})
export class ComprasAgenciaComponent implements OnInit {
    ...
    confirmarAprobar(compra: Compra): void {
        Swal.fire({
            ...
            html: `Se aprobarán las <strong>${compra.total_lineas}</strong> línea(s) solicitadas
                   por <strong>${this._escapar(compra.bodega)}</strong>.`,
            icon: 'question',
            input: 'textarea',
    ...
    confirmarRechazar(compra: Compra): void {
        Swal.fire({
            ...
            html: `Solicitud de <strong>${this._escapar(compra.bodega)}</strong>.`,
            icon: 'warning',
            input: 'textarea',
    ...
    /** Aprobada, Comprado o Requiere Autorización ya tienen mesa de trabajo que ver/continuar */
    ...
    irAMesaTrabajo(compra: Compra): void {
        this.router.navigate(['/compras2-mesa-trabajo', compra.id]);
    }
